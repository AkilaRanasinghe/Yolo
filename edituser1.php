...
<div class="row">
  <div class="coln1">
    <img src="images/user.jpg" style="margin: 50%" alt="user" width="200" height="200">
  </div>
  <div class="coln2">

  <?php
		
		$sql="SELECT * FROM register where Cus_ID = '".$uname."'";
		$result=$conn->query ($sql);
		if ($result -> num_rows >0)
		{
			while ($row= $result -> fetch_assoc())
        ...
        ?>
        <form method="POST" action="edituser.php" onsubmit="return password()">
          <table border=1 style="margin-left:160px;">
        <tr>
              <th>
        <xyz>First Name&nbsp;</xyz></th><td><input type="text" name="Fname" class="nm"value="<?php echo $fn ?>" required></th></tr>
        <tr>
              <th>
        <xyz>Last Name&nbsp;</xyz></th><td><input type="text" name="Lname" class="nm"value="<?php echo $ln ?>" required></th></tr>
        <tr>
              <th>
        <xyz>BirthDay&nbsp;</xyz></th><td><input type="Date" name="DOB" class="nm"value="<?php echo $Db ?>" required></th></tr>
            <tr>
              <th>
        <xyz>Country&nbsp;</xyz></th><td><input type="text" name="country" class="nm"value="<?php echo $cun ?>" required></th></tr>		
        <tr>
              <th>
        <xyz>Email&nbsp;</xyz></th><td><input type="text" name="Email" class="nm"value="<?php echo $EM ?>" required></th></tr>
        <tr>
              <th>
        <xyz>User Name&nbsp;</xyz></th><td><input type="text" name="Phone" class="nm"value="<?php echo $Phn ?>" required></th></tr>
        <tr>
              <th>
        <xyz>Password&nbsp;</xyz></th><td><input type="password" name="pwd" id="pwd" class="nm" pattern="(?=.+\d)(?=.+[a-z])(?=.+[A-Z]).{5-10}" value="<?php echo $PW ?>" required></th></tr>
        <tr>
              <th>
        <xyz>RE-Type-Password&nbsp;</xyz></th><td><input type="password" name="cnpwd" id="cnpwd" class="nm" placeholder="Confirm Password" required></th></tr></table>
     <input type="submit" id="sbt" value="SAVE" style="margin-top:20px;">
    </form>
  </div>
  </div>
</br>
