...
$sql = "SELECT * FROM item I,orderitem O WHERE I.Item_ID = O.Item_ID AND O.User_Name = '".$uname."'";
$result = $conn->query($sql);
if($result->num_rows > 0)
{
	while($row = $result->fetch_assoc())
	{
		...
		<td>".$row["Quantity"]."</td>
		<td>".$row["Price"]."</td>
		<td><center><button type='submit'><a href='pay.php?id=$id'>Make Payment</a></button></center></td>
		</tr>";
	}
}	
?>
