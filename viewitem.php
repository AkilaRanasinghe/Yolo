...
$sql = "SELECT * FROM item where item.Item_ID = '".$recordId."'";
...
if($result->num_rows > 0)
{
	while($row = $result->fetch_assoc())
	{
		...
		<br/><br><br><br>
		Quantity<select name='quantity' required>
		...
		<br><br><br>
		<center>
		<input type='checkbox' id='aditm' onclick='subenable()'>
		Add Product to Cart. 
		<br><br><br>
		<input type='submit' id='sbt' value='Add to Cart' disabled>
		</center>  
		</form>
		</div>";
	}
